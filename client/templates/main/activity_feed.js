...
angular
 ...
 function FeedsCtrl ($scope, $reactive, $state, $sce, $ionicLoading, $timeout) {
    ...
    this.go = function(id) {
        if ( Feeds.findOne({ _id: id }).linkType === "product" ) {
            $state.go('app.product', { listingId: Feeds.findOne({ _id: id }).linkID });
        }
        else if ( Feeds.findOne({ _id: id }).linkType === "mypost" ) {
            $state.go('app.myproduct', { listingId: Feeds.findOne({ _id: id }).linkID });
        }
        else if ( Feeds.findOne({ _id: id }).linkType === 'profile' ) {
            $state.go('app.myprofile', { profileId: Feeds.findOne({ _id: id }).linkID });
        }
        else {
            return;
        }
